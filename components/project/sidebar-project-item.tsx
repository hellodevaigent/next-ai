...
export function SidebarProjectItem({ project }: SidebarProjectItemProps) {
  const { open: sidebarOpen, setOpenMobile: setOpenSidebarMobile } = useSidebar()
    ...
    >
      {isEditing ? (
        ...
        <>
          ...
          >
            <div
              className="text-primary relative line-clamp-1 flex w-full items-center mask-r-from-80% mask-r-to-85% px-2 py-2 text-sm text-ellipsis whitespace-nowrap"
              title={displayName}
            >
              <span className="mr-2">
                <FolderIcon size={18} />
