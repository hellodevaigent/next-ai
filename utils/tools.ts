...
export const weatherTool = {
  ...
  execute: async ({ location }: { location: string }) => {
    // Di aplikasi nyata, Anda akan memanggil API cuaca di sini.
    // Untuk contoh ini, kita akan menggunakan data tiruan.
    const weatherData = {
      location,
...
export const erc20TokenTool = {
  ...
  parameters: z.object({
    tokenName: z.string().optional().describe('The name of the token, e.g., "Uniswap".'),
    tokenAddress: z.string().optional().describe('The contract address of the token.'),
    chainKey: z.string().optional().describe('The key of the chain, e.g., "eth-mainnet".'),
  }),
  execute: async ({ tokenName, tokenAddress, chainKey }: { tokenName?: string; tokenAddress?: `0x${string}`; chainKey?: string }) => {
    const GATEWAY_ENDPOINT = "http://localhost:5002/graphql";
    ...
    try {
      ...
      if (!result) {
        return { error: `No token data found for address: ${finalTokenAddress}` };
      }
